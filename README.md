...
| [clipbus-formatter-plugin](./clipbus-formatter-plugin/README.md) | Pretty-printer: detect JSON / XML / SQL and re-indent (or uppercase keywords), render CSV/TSV as a table, and parse a URL query string into a key/value table — formatted preview with one-click copy. |
| [clipbus-converter-plugin](./clipbus-converter-plugin/README.md) | Conversion toolkit: Unix timestamp ↔ date (local / ISO / UTC / relative), integer radix table (dec/hex/oct/bin + ASCII), ISO 8601 duration breakdown (human components + total seconds), temperature across Celsius / Fahrenheit / Kelvin, and a naming-case draft (camel/snake/kebab… 8 styles). |
