...
|--------|--------------|-------------|------|
| [clipbus-decoder-plugin](./clipbus-decoder-plugin/README.md) | `clipbus-decoder-plugin` | Encode/decode toolkit: detect & decode Base64, parse JWT (header/payload + expiry status), inspect a `data:` URI (media type / encoding / decoded size + text preview), plus a multi-codec draft (URL / HTML / Base64 / Unicode / JSON, both directions). | [GIF](./docs/gifs/decoder.gif) |
