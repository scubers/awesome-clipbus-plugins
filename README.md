...
| [clipbus-preview-plugin](./clipbus-preview-plugin/README.md) | Visual preview: detect color values (HEX / RGB / HSL + CSS names) into a swatch with WCAG contrast, render a CSS gradient (linear / radial / conic) into a live swatch, render Markdown into a safe HTML preview card, and turn a copied URL into a scannable QR code. |
...
| [clipbus-generator-plugin](./clipbus-generator-plugin/README.md) | Generate a UUID v4, a sortable ULID, or a strong random password (configurable length / case / digits / symbols / count) via Web Crypto, plus Lorem Ipsum placeholder text (paragraphs / sentences / words); live-preview and copy from a draft form. |
