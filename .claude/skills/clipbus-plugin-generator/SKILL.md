...
- 每个 feature：`payload.ts`（单一真相源）→ detector/renderer/action 运行时 → `app.vue`（renderer/draft 才有）→ **preview scenario 并接进 `PreviewShellApp.vue` 的 `RENDERERS`/`ACTIONS` 注册表（必做，不是可选）**；renderer 还要配 `clipbus.window.autoFit()` 搭 manifest `"auto"`/`{min,max}` 高度、长内容补内部滚动（见 authoring-guide §8/§10）。
...
- **preview 工作台必须 wire（不是可选）**：每个 renderer / draft action 都要在 `PreviewShellApp.vue` 里 `import` 其 `app.vue` 并登记进 `RENDERERS`/`ACTIONS` 注册表（key = feature 目录名 = `scenario.component`），`npm run dev` 看得到真实组件渲染才算完。scaffold 产出的是 wire-ready 壳（mock host bridge + topic 注入 + `<component :is>`），只需补 import + 注册 + 带 `component` 字段的 scenario。**留 `wire component in PreviewShellApp` 占位符 = 没做完**（真实踩过：整批插件 preview 全是死占位）。详见 authoring-guide §10。
- **非固定高度 renderer 必须调 autoFit**：manifest height 为 `"auto"` 或 `{min,max}` 时，UI 须 `onMounted` 调 `clipbus.window.autoFit({min,max})`（值与 manifest 一致）、`onUnmounted` 清理——不调则停在最小高、内容截断，**`npm run verify` 查不出**（隐蔽坑，真实踩过）。内容可能超 `max` 的列表/表格再套 `max-height + overflow-y:auto` 内部滚动；别在根 `<main>`/body 加 `overflow` 或 `height:100%`。详见 authoring-guide §8。
