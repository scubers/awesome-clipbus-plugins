...
- **Manifest 固定 schema v3**：顶层 `schemaVersion` 必须为 `3`；detector 与 Action 都用 `supportedInputKinds`。`supportedItemTypes`、`inputConstraints`、`possibleResultKinds` 均是 v2/legacy 字段，宿主不会自动适配。
