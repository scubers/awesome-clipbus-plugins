...
- **schema major bump 要迁移契约，不是只改 manifest 数字**：schema v2→v3 时，每个插件都要改 `schemaVersion`，每个 Action 都要把 `supportedItemTypes` 改成 `supportedInputKinds` 并清掉 legacy 字段；同时盘点 Action Runtime/UI 是否错误依赖原始 item。SDK 0.9+ 的 `sourceItem` 只保留身份，处理依据当前 `content.kind`；图片 Action 用 `host.action.materializeInputImagePath()` / `clipbus.asset.currentActionInputImageUrl()`。对应测试、preview `actionInput`、插件设计文档和 generator authoring 资产必须一起迁移。
