...
| 插件 | 子目录 | 描述 | 演示 |
|------|--------|------|------|
| [clipbus-decoder-plugin](./clipbus-decoder-plugin/README.md) | `clipbus-decoder-plugin` | 编解码工具：Base64 仅在解码为可读 UTF-8 文本时成功（忽略二进制与乱码结果）、JWT 解析（header/payload + 过期状态）、`data:` URI 检视（媒体类型 / 编码 / 解码大小 + 文本预览）、多编解码 draft（URL / HTML / Base64 / Unicode / JSON 双向）| [GIF](./docs/gifs/decoder.gif) |
