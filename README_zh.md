...
| [clipbus-converter-plugin](./clipbus-converter-plugin/README.md) | 转换工具：Unix 时间戳↔日期（本地 / ISO / UTC / 相对，含 9 时区世界时钟）、整数进制对照（dec/hex/oct/bin + ASCII）、ISO 8601 时长拆解（人类可读分量 + 总秒数）、温度三标换算（摄氏 / 华氏 / 开尔文）、数据大小换算（SI 与 IEC：KB/MB/GB 与 KiB/MiB/GiB + 精确字节）、命名风格转换 draft（camel/snake/kebab… 共 8 种）|
