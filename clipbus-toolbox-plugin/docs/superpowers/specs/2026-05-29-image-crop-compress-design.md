...
- **`clipbus.asset.currentActionInputImageUrl()`（UI Action context）→ `{url?}`**：返回不透明 `clipbus-asset://` 令牌 URL，`<img src>` 直接渲染当前级联 image 值，真实路径不进 JS（沙箱 block `file://`）。**这是 Action WebView 显示当前输入图的指定机制**——无需 base64、无需 runtime 往返。
...
- runtime 侧 `host.action.materializeInputImagePath()→{path}` 把当前级联图片复制到临时文件供 sharp 读取（invocation 结束宿主自动清理；同一 invocation 幂等）。
- **没有 `clipbus.ready()`**（0.3.0 移除）：模块加载即注册 `.on()`，`.current()` 取快照并用 `?.`/`??` 兜底。`autoFit` 在 `@clipbus/plugin-sdk/dom`。
- **无需新权限**：`materializeInputImagePath` / `allocateImageTempPath` / `asset.currentActionInputImageUrl` 均不受门控；本功能不写 attachment，故不需要 `setAttachment`。`manifest.permissions` 保持不变。
...
Draft UI（Vue WebView）：
   const { url } = await clipbus.asset.currentActionInputImageUrl(); img.src = url   ← 当前 Action 图片全分辨率直显
   ...
   clipbus.runtime.invoke("image-edit/process-image", { quality, crop:{x,y,width,height}(原图像素) })
        → runtime messageHandler：
             materializeInputImagePath() → sharp().metadata() 取真实格式
...
- **`messageHandlers["image-edit/process-image"](req, ctx)`**：
  1. `const { path: src } = await ctx.host.action.materializeInputImagePath();`
...
- **取图**：`onMounted` → `const {url}=await clipbus.asset.currentActionInputImageUrl(); if(url) previewUrl=url;` 否则显示错误占位。
...
3. `manifest.json` → `actions` 数组加一项（**无新权限**）：
   ```json
   { "id":"image-edit", "title":"Crop & Compress", "supportedInputKinds":["image"],
...
- 当前值非 image：`supportedInputKinds:["image"]` 已限制宿主只对当前图片值提供；`resolveSession` 与 handler 内再兜底。
- `currentActionInputImageUrl()` 返回空 / 图片加载失败：UI 显示错误占位，禁用「应用」。
- `materializeInputImagePath` / sharp 抛错（损坏图、不支持格式、crop 越界）：handler catch 抛出 → `invoke` reject → UI 显示错误并恢复按钮，不 `complete`。
...
- **Node 单测（无宿主，沿用 `node --test`）**：
  ...
  - `process.ts` 集成：用 `tests/fixtures` 小图 + mock `ctx.host`（`materializeInputImagePath`→fixture 路径、`allocateImageTempPath`→tmp 路径），断言输出尺寸 == crop、输出格式正确、低质量文件更小。
- **vite workbench（`npm run dev`，`?view=action`）**：新增带 `actionInput.kind = "image"` 的 Action preview scenario；`process-image` / `complete` 在 workbench 中记录调用。
